...
impl Workspace {
    // create workspace through username user id
    ...
    // from workspace table to get info
...
mod tests {
    ...
    async fn workspace_should_create_and_set_owner() -> Result<()> {
        ...
        // to create workspace super user workspace test user_id = 0
        let ws = Workspace::create("test", 0, &pool).await.unwrap();

        let input = CreateUser::new(&ws.name, "Json test", "json@acme.org", "Hunter42");
